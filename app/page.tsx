  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        ...
        <div className="top-actions"><label className="city-picker"><span>●</span><select value={city} onChange={event=>{setCity(event.target.value);if(event.target.value!=="Ankara")window.location.href=`/discover`}} aria-label="Şehir seç">{turkishCities.map(item=><option key={item}>{item}</option>)}</select></label><a className="icon-button" href="/notifications" aria-label="Bildirimler">♢<i /></a><a className="profile-button" href="/profile" aria-label="Profil">AY</a></div>
      </header>
